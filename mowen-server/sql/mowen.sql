CREATE DATABASE IF NOT EXISTS mowen DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;




DROP TABLE IF EXISTS `mowen`.`user`;

CREATE TABLE `mowen`.`user` (
...
DROP TABLE IF EXISTS `mowen`.`article`;

CREATE TABLE `mowen`.`article` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`comment`;

CREATE TABLE `mowen`.`comment` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`sort`;

CREATE TABLE `mowen`.`sort` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`label`;

CREATE TABLE `mowen`.`label` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `sort_id` int NOT NULL COMMENT '分类ID',
...
DROP TABLE IF EXISTS `mowen`.`tree_hole`;

CREATE TABLE `mowen`.`tree_hole` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `avatar` varchar(256) DEFAULT NULL COMMENT '头像',
...
DROP TABLE IF EXISTS `mowen`.`wei_yan`;

CREATE TABLE `mowen`.`wei_yan` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` int NOT NULL COMMENT '用户ID',
  `like_count` int NOT NULL DEFAULT 0 COMMENT '点赞数',
...
DROP TABLE IF EXISTS `mowen`.`web_info`;

CREATE TABLE `mowen`.`web_info` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`resource_path`;

CREATE TABLE `mowen`.`resource_path` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`resource`;

CREATE TABLE `mowen`.`resource` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` int NOT NULL COMMENT '用户ID',
  `type` varchar(32) NOT NULL COMMENT '资源类型',
...
DROP TABLE IF EXISTS `mowen`.`history_info`;

CREATE TABLE `mowen`.`history_info` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`sys_config`;

CREATE TABLE `mowen`.`sys_config` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`family`;

CREATE TABLE `mowen`.`family` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` int NOT NULL COMMENT '用户ID',
...
DROP TABLE IF EXISTS `mowen`.`im_chat_user_friend`;

CREATE TABLE `mowen`.`im_chat_user_friend` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` int NOT NULL COMMENT '用户ID',
...
DROP TABLE IF EXISTS `mowen`.`im_chat_group`;

CREATE TABLE `mowen`.`im_chat_group` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`im_chat_group_user`;

CREATE TABLE `mowen`.`im_chat_group_user` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT 'id',
...
DROP TABLE IF EXISTS `mowen`.`im_chat_user_message`;

CREATE TABLE `mowen`.`im_chat_user_message` (
...
DROP TABLE IF EXISTS `mowen`.`im_chat_user_group_message`;

CREATE TABLE `mowen`.`im_chat_user_group_message` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'id',
  `group_id` int NOT NULL COMMENT '群ID',
  `from_id` int NOT NULL COMMENT '发送ID',
...
INSERT INTO `mowen`.`family` (`id`, `user_id`, `bg_cover`, `man_cover`, `woman_cover`, `man_name`, `woman_name`, `timing`, `countdown_title`, `countdown_time`, `status`, `family_info`, `like_count`, `create_time`, `update_time`) VALUES (1, 1, '背景封面', '男生头像', '女生头像', 'ZS', 'WX', '2018-03-18 19:33:22', '倒计时', '2025-05-03 11:28:00', 1, '', 0, '2025-08-22 22:00:00', '2025-08-22 22:00:00');

INSERT INTO `mowen`.`im_chat_group` (`id`, `group_name`, `master_user_id`, `introduction`, `notice`, `in_type`) VALUES(-1, '公共聊天室', 1, '公共聊天室', '欢迎光临！', 0);

insert into `mowen`.`im_chat_group_user` (`id`, `group_id`, `user_id`, `admin_flag`, `user_status`) values(1, -1, 1, 1, 1);




INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (1, 'QQ邮箱号', 'spring.mail.username', '', '1');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (2, 'QQ邮箱授权码', 'spring.mail.password', '', '1');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (3, '邮箱验证码模板', 'user.code.format', '【MOWEN】%s为本次验证的验证码，请在5分钟内完成验证。为保证账号安全，请勿泄漏此验证码。', '1');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (4, '邮箱订阅模板', 'user.subscribe.format', '【MOWEN】您订阅的专栏【%s】新增一篇文章：%s。', '1');
...
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (12, '七牛云-bucket', 'qiniu.bucket', '', '1');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (13, '七牛云-域名', 'qiniu.downloadUrl', '仿照：【https://file.mowen.cn/】，将域名换成自己的七牛云ip或域名', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (15, 'IM-聊天室启用状态', 'im.enable', 'true', '1');
...
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (18, '公安备案号', 'gonganbeian', '', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (19, '前端静态资源路径前缀', 'webStaticResourcePrefix', '/static/', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (20, '导航栏菜单显示', 'bar.menu.show', '首页,记录,家,随笔,旅拍,百宝箱,留言,联系我,后台', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (21, '邮箱通知启用状态', 'mail.enable', 'true', '1');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (22, '友链-网站名称', 'friendWebName', 'MOWEN', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (23, '友链-网址', 'friendUrl', 'http://blog.zsawxotof.site', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (24, '友链-头像', 'friendAvatar', 'http://qncdn.zsawxotof.site/webAvatar/zhangshuang117553345219565.jpg', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (25, '友链-描述', 'friendIntroduction', '玄牝之门，是谓天地根。欢迎来到代码的世界！', '2');
INSERT INTO `mowen`.`sys_config` (`id`, `config_name`, `config_key`, `config_value`, `config_type`) VALUES (26, '友链-网站封面', 'friendCover', 'http://qncdn.zsawxotof.site/webBackgroundImage/zhangshuang11755334510431677.jpg', '2');
