## [MOWEN](http://blog.zsawxotof.site) - 玄牝之门，是谓天地根
...
- 后端（SpringBoot）：
  ...
  - 导入SQL文件`mowen.sql`到数据库

  - 配置数据库连接（application.yml里面的datasource信息）

  - 启动（PoetryApplication）

- 登录**网站后台（[http://localhost/admin）](http://blog.zsawxotof.site/admin)**-** 配置管理**可以配置网站信息。请注意部分配置信息修改后需要重启Java，比如邮箱号和邮箱授权码。
